...
load("Data_Needed_for_Lateral_Analysis_Ver1.0.Rdata")
...
lateral.snake.gpa <- gpagen(mydata.new, curves = mysliders, ProcD = F) # alignment with bending energy
snake.gdf <- geomorph.data.frame(lateral.snake.gpa)
...
### Based on the tiny R^2 value (0.02199), I can say that only 2.199% of variation in morphology is due to the size of the snake
### and that due to the large P value, it is also a non significant result
...
phy.data.frame <- geomorph.data.frame(coords = lateral.mean.PCA.data$coords,
                                      phy = tree_ult,
                                      group = lateral.mean.PCA.data$genus)
...
phy1 <- physignal(lateral.mean.PCA.data$coords, tree_ult)
summary(phy1)
...
### I will need a GPA before running the analysis. For example, 'lateral.snake.gpa'.

### Single-Factor MANOVA ###
#create geomorph data frame
### Using species and country as a grouping variable
snake.gdf <- geomorph.data.frame(lateral.snake.gpa, gp = as.factor(paste(info$species, info$Country, sep=", ")))
...
snake.gdf <- geomorph.data.frame(lateral.snake.gpa, gp = as.factor(paste(info$Genus, info$Country, sep=", ")))
...
snake.gdf <- geomorph.data.frame(lateral.snake.gpa, gp = as.factor(info$Genus))
...
snake.gdf <- geomorph.data.frame(lateral.snake.gpa, gp = as.factor(info$Country))
...
snake.gdf <- geomorph.data.frame(lateral.snake.gpa, gp = as.factor(info$species))
...
###################################################
# 5: 2B-PLS for analysing lateral and lateral view #
###################################################

### Make sure to run both lateral and lateral tests first

### Note that this analysis used the individual sample level data

lateral.dorsal.2BPLS <- two.b.pls(lateral.snake.gpa$coords, dorsal.snake.gpa$coords, iter = 999, print.progress = T) 

### Error ^: Unequal number of specimens in each sample
### Find a script that can drop non matching entries...

print(lateral.dorsal.2BPLS)
plot(lateral.dorsal.2BPLS)

### Note that this analysis used mean data

lateral.dorsal.means.2BPLS <- two.b.pls(lateral.mean.PCA.data$coords, dorsal.mean.PCA.data$coords, iter = 999, print.progress = T)
print(lateral.dorsal.means.2BPLS)
plot(lateral.dorsal.means.2BPLS)
